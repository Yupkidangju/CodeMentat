...
- startup recovery는 단순히 `open()`이나 wall-clock heartbeat만으로 stale을 가정하면 안 된다. DB보다 먼저 crash-release OS lock을 획득한 process만 Prepared와 orphan Streaming을 복구해야 하며, busy/permission은 corruption과 별도 분류해야 한다.
