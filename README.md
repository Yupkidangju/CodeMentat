...
6. **Egress Consent & 프라이버시 쉴드**
   ...
   - AppData DB는 DB open 전 OS process lock으로 한 runtime만 사용할 수 있으며 두 번째 실행은 live DB를 격리하거나 복제하지 않고 저장소 open을 거부합니다.
...
├── mentat-platform         # OS 앱데이터 격리, 자격증명, process-lifetime DB lock
