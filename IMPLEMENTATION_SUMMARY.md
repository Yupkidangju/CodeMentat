...
sequenceDiagram
    ...
    alt 로컬 슬래시 명령 (/onboard, /structure, /conflicts, /where, /risks)
        ...
        App->>Analysis: EgressFilter::assemble_packet_with_user_exclusions(files, query, user_exclusions) (Query-Aware 파일 랭킹, Bearer/고엔트로피 마스킹 및 Exact Preview)
        Analysis-->>App: EgressPacket (민감정보 배제 + SHA-256 해시)
        App->>Dev: Egress Consent Sheet 표시 (포함 파일 및 행 수 미리보기, 제외 체크박스)
...
| `mentat-repository` | 읽기 전용 저장소 엔진 | `ReadOnlySession` (Single-Scan `create_snapshot_from_files`, 결정적 정렬 스냅샷 다이제스트, 100k 파일/2GiB 예산 통제), `.gitignore` 필터링, 파일 메타데이터 및 스트리밍 해시, `inspect_file` 심볼릭 정규화 가드, `RepositoryWatcher` (1000ms I/O 쓰로틀링 & `TreeSignature` 완전성 감지) | `test_read_only_session_scan_and_snapshot`, `test_external_path_blocked`, `test_sec_f006_inspect_file_canonical_safety`, `test_dbg_f003_scan_and_snapshot_deterministic_benchmark`, `test_dbg_f008_watcher_throttling_and_change_detection` |
...
| `mentat-analysis` | 정적 분석 및 유출 통제 | `ProjectDetector`, `SemanticKernel` (/onboard, /structure, /conflicts, /where, /risks 워크플로별 `EvidenceRef` 연계), `EgressFilter` (고엔트로피/Anthropic/HuggingFace/Slack/Bearer/AWS/JWT/PEM 비밀정보 마스킹, 사용자 제외 파일 지원), `ApprovedInferenceRequest` (단일 소비 계약) | 14개 테스트 패스 |
...
| `mentat-inference-openai` | 멀티 프로바이더 스트리밍 | Google Gemini (`x-goog-api-key`), OpenRouter (`Bearer`), 사전 응답 취소, 바이트 버퍼 기반 SSE 스트리밍, Fail-Closed 누락 키, 취소 및 Invalid URL 검증 | `test_multi_provider_adapter_default_initialization`, `test_gemini_and_openai_missing_key_infer_fail_closed`, `test_pre_response_cancellation_aborts_immediately`, `test_adapter_invalid_url_health_check_fail_closed` |
...
| `mentat-app` | eframe 데스크톱 UI | 3단계 점진적 공개 위젯, 비동기 프리뷰 및 UI 리페인트 웨이크업, 채널 오류/단절 복구 (Stream Disconnect 터미널 처리 포함), 최근 저장소 원클릭 재오픈, 사용자 파일 제외 토글, 클라우드 응답 주장 정규화, 뷰포트 크기 자동 조절 | 2개 테스트 패스 |

---

## 3. 품질 게이트 검증 결과 (Verification Results)

- **단위/회귀 테스트:** 전체 38개 테스트 통과 (`cargo test --workspace --locked`)
