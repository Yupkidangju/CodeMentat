...
sequenceDiagram
    ...
    App->>Dev: 답변 렌더링 (관찰/추론/제안 태그 + 소스코드 증거 링크)
```
...
| `mentat-repository` | 읽기 전용 저장소 엔진 | `ReadOnlySession`, `.gitignore` 필터링, 파일 메타데이터 및 스트리밍 해시, `inspect_file` 심볼릭 정규화 가드, `RepositoryWatcher` | `test_read_only_session_scan_and_snapshot`, `test_external_path_blocked`, `test_sec_f006_inspect_file_canonical_safety` |
| `mentat-storage` | AppData SQLite 영속화 | `SqliteStorage`, 최근 저장소 목록 관리 (`mentat.db`), 스키마 자동 마이그레이션 | `test_sqlite_storage_save_and_list_recent_repos` |
| `mentat-analysis` | 정적 분석 및 유출 통제 | `ProjectDetector`, `SemanticKernel` (/onboard, /structure, /conflicts, /where), `EgressFilter` (고엔트로피/AWS/JWT/PEM 비밀정보 마스킹, 정확한 행 수 미리보기), `ApprovedInferenceRequest` (단일 소비 계약) | 12개 테스트 패스 |
...
| `mentat-inference-openai` | 멀티 프로바이더 스트리밍 | Google Gemini (`x-goog-api-key`), OpenRouter (`Bearer`), 사전 응답 취소 및 바이트 버퍼 기반 SSE 스트리밍 | `test_multi_provider_adapter_default_initialization` |
...
| `mentat-app` | eframe 데스크톱 UI | 3단계 점진적 공개 위젯, 비동기 프리뷰 및 UI 리페인트 웨이크업, 채널 오류/단절 복구, 뷰포트 크기 자동 조절 | 2개 테스트 패스 |

---

## 3. 품질 게이트 검증 결과 (Verification Results)

- **단위/회귀 테스트:** 전체 29개 테스트 통과 (`cargo test --workspace --locked`)
- **포맷팅 검사:** `cargo fmt --all -- --check` (0 diffs)
- **정적 분석:** `cargo clippy --workspace --all-targets --all-features -- -D warnings` (0 errors, 0 warnings)
