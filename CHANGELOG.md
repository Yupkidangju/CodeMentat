...
### Changed
- crash 후 runtime 작업이 없는 Pending/Streaming assistant message는 OS lock 획득 뒤 startup recovery transaction에서 `INTERRUPTED_BY_RESTART` Failed로 닫는다.
...
### Security
- DB open/migration 전 Windows share-mode 0 또는 Unix `flock` process-lifetime lock을 추가해 live 동시 handle/process를 거부한다. force-kill 직후 첫 reopen만 Prepared receipt와 orphan turn을 복구한다.
...
### Fixed
- **Gemini 모델 활성화 probe:** 1-token 제한과 첫 응답 part 고정 파싱을 제거하고 모든 candidate/part의 text 및 finish/thinking 진단을 검증해 정상 모델이 비호환으로 남던 문제를 수정했다.
...
### Added
- **멀티 플랫폼 빌드 오케스트레이터:** 메뉴형·명령형을 함께 제공하는 Rust `xtask`, `cargo mentat-build` alias, Windows PowerShell/POSIX shell wrapper와 target preflight/dry-run을 추가했다.
